...
public class PropertyManagerTests
{
    ...
    public PropertyManagerTests()
    {
        ...
        SetupContentLoader(_contentLoader);
        _sut = new PropertyManager(
            new PropertyNameStrategy(),
            new PropertyResolver(),
            new PropertyHandlerService()
        );
        _page = new StandardPageBuilder().Build();
    }
    ...
    {
        // Arrange
        var dateTimes = new List<DateTime> { new(2000, 1, 12), new(3000, 1, 20) };
